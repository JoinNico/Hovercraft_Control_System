...
\subsection{软件总体设计}
% \subsection{主控制器软件系统}
基于气垫船运动控制的复杂性与实时性需求，本系统设计并实现了一套分层化、模块化的主控制器软件架构。该架构自上而下划分为业务逻辑层、功能中间件层及硬件驱动层，各层之间通过标准化的API接口进行解耦与通信。在业务逻辑层，系统整合了基于视觉的图像识别模块与实时元素决策模块，前者通过摄像头采集环境信息并识别关键航路点与障碍物，后者则依据识别结果、IMU姿态数据以及预设航迹规划，通过改进型PID算法生成精确的电机控制指令。同时，该层集成了用户交互（UI菜单）与无线数据传输（蓝牙）功能，实现了控制指令下发与船体状态信息的双向通信。

在底层实现上，硬件驱动层对电机、OLED显示屏、摄像头、IMU等外设进行了统一的抽象与封装，提供了稳定可靠的设备操作接口。功能中间件层则构建了包括OTSU图像二值化、数字滤波器、PID控制器以及VOFA+上位机调试协议在内的核心算法库与工具集，为上层业务提供高效的计算与通信支持。整个系统运行在实时操作系统或精心设计的多线程调度机制之上，确保关键控制任务（如电机PWM输出、IMU数据读取）的时序确定性，同时通过DMA、硬件定时器等片上资源优化，大幅降低了CPU负载，保障了气垫船在各种工况下的快速响应与稳定航行。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=1.2]{./pic/c2.png}
    \caption{系统软件架构图}
    \end{figure}

\subsection{软件流程设计}
气垫船控制系统采用模块化分层设计，以实时操作系统为调度核心，构建了一套完整、高效的控制流程。系统上电后，首先完成时钟、通信接口、传感器（包括IMU、摄像头、编码器等）及执行机构（电机、舵机）的初始化，随后启动FreeRTOS实时内核。操作系统创建并调度多个优先级分明的并行任务：高速传感器采集任务（10ms周期）实时获取姿态、速度与视觉数据；决策任务对多源信息进行融合分析，完成路径规划与控制指令生成；电机控制任务根据指令精确调节推进器与升力风扇的PWM输出；人机交互任务（30ms周期）负责图形界面刷新与按键响应。各任务间通过信号量、队列等机制实现安全通信与同步，形成“感知-决策-执行”的闭环控制流，并通过系统心跳任务监控各模块运行状态，确保气垫船在复杂环境下能够稳定、灵敏地完成悬浮、转向与航迹跟踪等控制目标，如图\ref{main-liu}所示。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.8]{./pic/chapter03/main-liu.png}
    \caption{系统软件流程图}
	\label{main-liu}
    \end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth ]{./pic/pid1.png}
    \caption{PID控制框图}
    \label{PID1}
    \end{figure}
...
在本系统中，方向环使用了串级PID将角度环和角速度环串联起来，从而达到更高的响应速度和稳定性。其控制框图如图\ref{motor}所示。
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{./pic/motor.png}
    \caption{行进电机PID控制框图}
    \label{motor}
    \end{figure} 
...
气垫船的横向电机采用基于图像偏差的位置PID控制策略，如图\ref{motorside}所示，通过实时采集的实际图像偏差与期望图像偏差（设定为0，即目标巡线中线位置）进行比较，将偏差值作为PID控制器的输入；控制器根据偏差的大小、累积和变化率计算出横向电机的控制量，动态调节横向推进力的大小和方向，从而产生精确的转弯向心力——当实际图像偏右时，横向电机向左提供推力，反之则向右提供推力，使气垫船能够快速、平稳地校正航向偏差，实现对预定路径的准确跟踪和稳定过弯。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=1.0]{./pic/motorside.png}
    \caption{横向电机PID控制框图}
    \label{motorside}
    \end{figure}

\subsection{数据采集任务}
\subsubsection{加权滑动平均滤波}
加权滑动平均滤波算法是一种经典的滤波方法。其主要思路是对信号进行滑动窗口处理，窗口内的数据进行加权平均化，以得到平滑后的信号。这样可以有效地去除周期性噪声和高频噪声，同时保留信号的整体趋势。加权滑动平均滤波的归一化数学表达式如下：
$$
\text{WMA}_t=\frac{\sum_{i=1}^n{w_i}X_{t+1-i}}{\sum_{i=1}^n{w_i}}
$$
式中，$X_t$为$t$时刻的观测值，$w_i$为第$i$个数据点的权重，$n$为滑动窗口的长度。
\subsubsection{ADC 数据处理}
在气垫船控制系统中，ADC（模数转换器）采集模块负责将各关键模拟量转换为数字信号以供处理器处理。系统通过ADC1的通道9（对应引脚B1）以8位分辨率对电源电压进行周期性采样，将模拟电压值量化为0-255的数字范围。采样数据经过加权滑动窗口滤波算法处理，该算法维护一个固定长度的数据队列，对新采集的数据赋予较高权重，对历史数据赋予递减权重，通过加权平均计算输出平滑稳定的电压值。这一处理机制有效抑制了电源噪声和瞬时干扰，为系统提供了可靠的电量监测，确保在电压异常时能够及时采取保护措施，保障气垫船的安全稳定运行，如图所示\ref{adc-liu}。
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.9]{./pic/adc-liu.png}
	\caption{ADC 采集与滤波处理流程图}
	\label{adc-liu}
	\end{figure}
...
\subsubsection{惯性运算单元数据处理}
惯性运算单元(IMU)数据处理过程首先采集加速度计和陀螺仪的原始数据，并进行单位转换、零偏校正和噪声滤除等预处理；随后进入卡尔曼滤波的核心迭代流程，该流程基于系统模型进行状态预测和误差协方差预测，再通过计算卡尔曼增益来权衡预测值与实际测量值的可信度，进而完成状态更新和协方差更新，如此循环迭代不断优化估计精度；最终输出经过滤波处理的平滑、稳定的姿态数据，这些优化后的数据被实时应用于姿态解算、运动状态估计以及后续的导航与控制决策中，有效抑制了传感器固有的噪声和漂移问题，如图\ref{imu-liu}所示。
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{./pic/imu-liu.png}
	\caption{IMU 数据处理流程图}
	\label{imu-liu}
	\end{figure}

\subsection{屏幕渲染任务}
屏幕渲染任务是一个周期性的界面更新线程，以周期循环执行，负责根据系统当前状态动态渲染用户界面。任务采用基于索引的UI管理机制，通过遍历预定义的UI列表，将当前激活的UI索引与列表中各元素的索引进行匹配，当匹配成功时便调用对应的回调函数，完成特定界面元素的绘制与状态更新。该设计实现了界面逻辑的模块化管理，确保仅渲染当前需要显示的界面组件，从而在满足界面流畅响应的同时，有效降低了处理器的负载，如图\ref{gui-liu}所示。
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.8]{./pic/gui-liu.png}
    \caption{屏幕渲染任务流图}
	\label{gui-liu}
    \end{figure}
% \subsection{电子调速器软件系统}
...
% \subsubsection{系统初始化}
% 系统启动初期，主函数先对 CH32 芯片的各个外设模块，如定时器、比较器、PWM、ADC 等进行初始化配置，以确保系统的正常运行。同时，对软件资源，如变量初始化、PWM 占空比默认值、电机死区设置等进行默认处理，确保系统运行时的数据一致性，以避免在 H 桥驱动电机时出现上下桥臂同时导通的情况，确保电机的安全运行。此外，其他如中断优先级的设置、串口通信的初始化等，都在系统启动的初期得到合理配置，为后续系统的稳定运行奠定基础。
...
% \section{无线遥控模块软件设计}
%     \subsection{系统软件架构设计}
%     \subsection{用户输入采集与处理}
%         \subsubsection{基于ADC的模拟摇杆数据采集}
%         \subsubsection{多按键扫描与状态检测}
%     \subsection{用户界面设计与实现}
%         \subsubsection{OLED-UI-Astra框架移植与初始化}
%         \subsubsection{界面布局、控件与状态显示}
%         \subsubsection{菜单逻辑与用户交互处理}
%     \subsection{无线通信协议与数据传输}
%         \subsubsection{LwPKT协议栈集成与配置}
%         \subsubsection{遥控数据帧封装与发送}
%         \subsubsection{接收数据解析与状态同步}
%     \subsection{主控程序逻辑与多任务调度}
%         \subsubsection{主循环与中断服务程序设计}
%         \subsubsection{各功能模块的协同与数据流}
%         \subsubsection{低功耗与稳定性设计考量}